...
public class Initial {
	...
	public static void selectWujiang(){
		...
		for (int j = 0; j < wujiangArray.length; j++) {
			if((j+1) == num){
				Character character = new Character();
				character.setName(wujiangArray[j][0]);
				character.setSkill(wujiangArray[j][2]);
				character.setMaxhp(Integer.valueOf(wujiangArray[j][1]));
				//存入Player类
				player = new Player();
